...
make_V_list <- function(k, g) {
  ...
  values2[values2 < 0] <- 0
  V = list()
  V[[1]] <- matrix(values1,   nrow = k, ncol = g)
  V[[2]] <- matrix(values2,   nrow = k, ncol = g)
  return(V)
...
make_W <- function(k, g) {
  ...
  W <- matrix(runif(k*g, 2,4),   nrow = k, ncol = g)
...
make_U_list <- function(k, z_rna, z_prot) {
  ...
  U[[1]] <- matrix(rnorm(z_rna*k, 2, 0.01),  nrow = k, ncol = z_rna)
  U[[2]] <- matrix(rnorm(z_prot*k, 2, 0.01),  nrow = k, ncol = z_prot)
  return(U)
