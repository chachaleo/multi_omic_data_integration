...
make_H_list <- function(n_rna, n_prot, k) {
  ...
  H_norm_1 <- matrix(runif(n_rna,10,20), nrow = n_rna, ncol = 1)
  H_norm_1 <- cbind(H_norm_1,matrix(runif(n_rna,20,30), nrow = n_rna, ncol = 1))
  H_norm_1 <- cbind(H_norm_1,matrix(runif(n_rna,30,40), nrow = n_rna, ncol = 1))
  
  H_norm_2 <- matrix(runif(n_prot,10,20), nrow = n_prot, ncol = 1)
  H_norm_2 <- cbind(H_norm_2,matrix(runif(n_prot,20,30), nrow = n_prot, ncol = 1))
  H_norm_2 <- cbind(H_norm_2,matrix(runif(n_prot,30,40), nrow = n_prot, ncol = 1))
